...
<!-- retirado de https://www.uio.no/studier/emner/matnat/math/STK3100/oppgaver/ -->

1. Suponha que $U_i \sim N\left(0, \sigma_u^2\right)$ e que dado $U_i = u_i$, as variáveis aleatórias binárias $Y_{ij}$ são independentes com 
$$
P\left(Y_{i j}=1 \mid U_i=u_i\right)=1-P\left(Y_{i j}=0 \mid U_i=u_i\right)=\Phi\left(\beta_0+\beta_1 x_{i j}+u_i\right).
$$
Aqui $\Phi$ é a distribuição normal acumulada padrão, e os $x_{i j}$ são valores de alguma covariável conhecida. Este é um modelo multinível probit. Mostre que 
$$
P\left(Y_{i j}=1\right)=1-P\left(Y_{i j}=0\right)=\Phi\left(\gamma_0+\gamma_1 x_{i j}\right),
$$
exibindo $\gamma_0$ e $\gamma_1$ em termos de $\beta_0$, $\beta_1$ e $\sigma_u^2$.

2. Agora, suponha que $U_i \sim N\left(0, \sigma^2_u\right)$ e que condicionalmente a $U_i = u_i$, as variáveis aleatórias $Y_{ij}$ são independentes e seguem uma distribuição gama com $\mathrm{E}\left[Y_{i j} \mid U_i = u_i\right]=$ $\exp \left(\beta_0+\beta_1 x_{i j}+u_i\right)$. Encontre uma expressão para a média marginal $\mathrm{E}\left[Y_{i j}\right]$ e para a variância marginal $\mathrm{Var}\left[Y_{i j}\right]$.
