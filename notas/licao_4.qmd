...
3.  Considere um outro estimador \underline{não-viesado} de $\boldsymbol{\beta}$: $\boldsymbol{\tilde{\beta}} = \boldsymbol{M}\boldsymbol{y}$, onde $$\boldsymbol{M} = (\boldsymbol{X}^T\boldsymbol{X})^{-1}\boldsymbol{X}^T + \boldsymbol{D},$$ e $\boldsymbol{D}$ é uma matriz $P \times n$ cujas entradas são não-zero. Mostre que $\boldsymbol{R} := \operatorname{Var}(\boldsymbol{\tilde{\beta}}) - \operatorname{Var}(\boldsymbol{\hat{\beta}})$ é positiva-definida.
