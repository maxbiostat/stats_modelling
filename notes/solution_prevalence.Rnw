...
\section*{Soluções}
  \begin{itemize}
    \item[a)]
      \begin{itemize}
        \item Uma primeira ideia é a distribuição beta que já conversamos em aula.
        \item Procurar usadas na biografia.
        \item O que acham de usar a \href{https://en.wikipedia.org/wiki/Truncated_normal_distribution#Definitions}{distribuição normal truncada}?
      \end{itemize}
  \end{itemize}
